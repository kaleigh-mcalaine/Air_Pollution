...
// constants and reference matricies
    ...
    // These are matricies of the Pasquill Gifford Stability class as a function of time of day, cloudiness, and wind speed, as found here: https://www.ready.noaa.gov/READYpgclass.php
    ...
    // converts the numeric Pasquill Gifford stability class into its corresponding letter
    string Numeric_to_Alphabet[6] = {"A", "B", "C", "D", "E", "F"}; //0=A 1=B 2=C 3=C 4=D 5=E 
    // string in a for loop that changes the description of the input direction (x = downwind, y = perpindicular to wind, z = vertical)
    string direction[3] = {"downwind (x)", "perpendicular to the wind (y)", "vertical (z)"};
    // matricies describing the upper limit of the distance (x-dir) and the corresponding parameters for each stability class - used to calculate sigma z i.e. col 1 = x max, col 2 = var a, col 3 = vol b 
    // source: //https://www3.epa.gov/scram001/userg/regmod/isc3v2.pdf
...
// variables
    ...
    double sigma_y;                                                  // METERS
    double sigma_z;                                                  // METERS
    double var_a, var_b, var_c, var_d;
    double theta; 
    int row_num;
    double x_dist_limit;                                             // KILOMETERS
    double conc, conc_ug;                                            // GRAMS/METERS^3 & KILOGRAMS/METERS^3
    bool check_num_output;
    double emissions_strength;                                       // GRAMS/SECOND   
    double x_m, y_m, z_m, x_km;                                      // METERS
    double between[3] = {x_m, y_m, z_m}; 
    double height_effective_stack, height_stack, height_plume_rise;  // METERS effective height of emissions stack = stack height + plume rise   
    double plume_wind_speed;                                         // METERS/SECOND
    double surface_wind_speed;                                       // METERS/SECOND
...
// user input variables in order of use
    ...
    double end_program; // hit any key to end the program (for users without their IDE set up to keep window open after running)

    
double find_stability_class(int wind) // returns Pasquill Gifford Stability Class based on user input (time of day, solar radiation, cloud cover) - classifications are based on 
{
...
double calc_conc_w_reflection(double x_m, double y_m, double z_m, double emissions_strength, double height_effective_stack, double plume_wind_speed, double sigma_y, double sigma_z) // calculates the concentration at a point (x.y.z) from the elevated point source
{
...
double get_sigma_y_vars(int class_num) // get variables c & d as a function of x distance and pasquill gifford stability class
{
    switch(class_num)
    { // all constants come from Table 1-1: "PARAMETERS USED TO CALCULATE PASQUILL GIFFORD FY" from the EPA: //https://www3.epa.gov/scram001/userg/regmod/isc3v2.pdf
        case 0: // A
            var_c = 24.1670;
            var_d = 2.5334;
            break;
...
double calc_sigma_y(double var_c, double var_d, double x_dist) // calculates sigma y from variable c, d, and x distance
{
...
double get_sigma_z_vars(double x_dist, int class_num) // get variables a & b as a function of x distance and pasquill gifford stability class
{
    switch(class_num)
    {
        case 0: // A
        ...
        case 2: // C - var a & var b are the same at all distances
            var_a = 61.141;
            var_b = 0.91465;
            break;
        case 3: // D
        ...
        default:
            cout << "Error with Pasquill Stability Class Calculations. Please try again." << endl;
            return 0;
    }
 return class_num;
}

double calc_sigma_z(double var_a, double var_b, double x_dist) // calculates sigma z fom variable a, b, x distance 
{
    sigma_z = var_a * pow(x_dist, var_b);
    return sigma_z;
}

bool check_num(string name) // checks if a user input is a number or numericallly related
{
...
// MORE INFORMATION - asks users if they would like more information -> what kind of information
...
// DISPERSION COEFFICIENT INPUTS / STABILITY CLASS CALCULATIONS
    ...
    if (input_sigmas == "n") // user does not have dispersion coefficient --> proceed to calculate with stability class
    {
        ...
        { 
            ...
            // assigns the integer wind_array_index based on wind speed category (0-2m/s, 2-3. 3-5, 5-6, 6+) (categories from wind speed are based on NOAA's Pasquill Gifford Stability class chart)
